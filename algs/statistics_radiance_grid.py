...
class StatisticsRadianceGrid(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterVectorDestination(self.OUTPUT_STAT, self.tr('Statistics Radiance'), type=QgsProcessing.SourceType.TypeVectorAnyGeometry))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        if self.inputRaster.bandCount() == 1:
            ...
            outputs['CalculRasterTotalRadiance'] = qgsTreatments.applyRasterCalcABC(outputs[self.SLICED_RASTER], outputs[self.SLICED_RASTER], outputs[self.SLICED_RASTER], parameters[self.RED_BAND_INPUT],parameters[self.GREEN_BAND_INPUT], parameters[self.BLUE_BAND_INPUT], self.outputRasterRadiance, formula, out_type=Qgis.DataType.UInt16, context=context,feedback=feedback)
